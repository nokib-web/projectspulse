                ...
                <div className="space-y-8 animate-in fade-in duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <Card className="flex items-center space-x-4 border-l-4 border-l-red-500">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-gray-900">{data?.stats.highRiskProjects}</p>
                            </div>
                        </Card>
                        <Card className="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-gray-900">{data?.stats.pendingCheckins}</p>
                            </div>
                        </Card>
                    </div>
                        ...
                        <Card title="Critical Projects">
                            <div className="space-y-4">
                                {data?.highRiskProjects.length === 0 ? (
                                    <p className="text-sm text-gray-500">No high-risk projects currently.</p>
                                ) : (
                                    data?.highRiskProjects.map((p) => (
                                        <div key={p.id} className="flex items-center justify-between p-3 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors">
                                            <div>
                                                <Link href={`/admin/projects/${p.id}`} className="text-sm font-bold text-gray-900 hover:text-indigo-600">
                                                    {p.name}
                                                </Link>
                                                <p className="text-xs text-red-500 mt-1 font-medium">{p.highRisks} High Severity Risks</p>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-xs text-gray-500 mb-1 font-medium">Health Score</div>
                                                <div className={`text-sm font-bold ${p.healthScore < 60 ? 'text-red-600' : 'text-yellow-600'}`}>
                                                    {p.healthScore}/100
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <Card title="Check-in Alerts">
                        <div className="space-y-3">
                            <p className="text-sm text-gray-600 mb-4">Projects with missing employee check-ins this week:</p>
                            {data?.missingCheckins.length === 0 ? (
                                <div className="p-8 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                    <p className="text-sm text-gray-500 font-medium italic">All projects have check-ins submitted!</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {data?.missingCheckins.map((p) => (
                                        <Link
                                            ...
                                            className="flex items-center p-4 rounded-xl bg-orange-50 border border-orange-100 text-orange-800 hover:bg-orange-100 transition-colors group"
                                        >
